...
def explore_for_project(cfg, led, pol, item, role, busy=(), size=None,
    ...
    """Deterministic D33 exploration, using pick's ordinary eligibility gates."""
    ...
    samples = [a for r in matching for a in r["attempts"]]
    # With no usage yet, a neutral one-million input/output token estimate
    # ranks known prices. Unknown models remain behind every priced model.
    tokens = {}
    for k in ("in", "cached", "out", "reasoning"):
        values = [a[f"tokens_{k}"] for a in samples if a[f"tokens_{k}"] is not None]
        tokens[k] = median(values) if values else (1e6 if k in ("in", "out") else 0)
    choices = []
    for name in order:
        ...
        row = next((r for r in matching if (r["platform"], r["model"], r["effort"])
        ...
        if candidate:
            ...
            weight = cfg.get("quota_groups", {}).get(pc.get("quota_group", name), {}).get("cost_weight", 1)
