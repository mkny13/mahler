...
class ExplorationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.cfg['routing'] = {role: ['normal', 'unknown', 'cheap']
                               for role in ('build', 'fix', 'sort', 'plan')}
        self.cfg['prices'] = {'cheap-model': {'in': 1, 'out': 2},
                              'normal-model': {'in': 4, 'out': 8}}
        self.cfg['projects']['p'] = dict(enabled=True, hot_hold=False,
                                         path='/tmp/test', repo='x/y')
    ...
    def pick(self, role='build', size='s', **kw):
        return router.explore_for_project(self.cfg, self.led, self.pol,
                                          self.led.item('p', 1), role,
                                          size=size, **kw)

    def seed(self, platform, count=8, **kw):
        for _ in range(count):
            self.led.create_run(
                project='p', number=1, role='build', size='s', platform=platform,
                model=self.cfg['platforms'][platform].get('model'), effort='default',
    ...
    def test_deterministic_hash_and_role_share(self):
        ...
        for attempts in range(40):
            ...
            selected = (int(hashlib.sha1(
                f'p#1:build:{attempts}'.encode()).hexdigest(), 16) % 1000 < 150)
    ...
    def test_peak_min_size_and_priority_route_are_enforced(self):
        ...
        with mock.patch.object(router, 'peak_state', return_value=(
                True, self.led.now() + timedelta(hours=1))):
            self.assertEqual(self.pick(), 'normal')
                ...
                with mock.patch('mahler.platforms.available', return_value=True), \
                    ...
                    if review:
                        ship._review_triggered_fix(
                            ctx, 'p', self.led.item('p', 1), 12, view, 'fix button')
                    else:
    ...
    def test_free_stream_one_size_up_never_two(self):
        ...
        for window in router.WINDOWS:
            self.led.record_usage('cheap', window, 0,
                                  iso(self.led.now() + timedelta(hours=1)))
    ...
    def test_no_unproven_falls_through(self):
        ...
        self.assertEqual(router.pick_for_project(
            self.cfg, self.led, self.pol, 'build', size='s')[0], 'normal')
    ...
    def test_start_persists_real_size_and_console_copy(self):
        ...
        with mock.patch('mahler.tick.runner.prepare', return_value={}), \
            ...
            self.assertTrue(tick.start(ctx, 'p', self.led.item('p', 1),
